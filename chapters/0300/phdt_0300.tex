...
Results include the probability density function of these reductions, from which derived descriptive statistics were computed (e.g.~the spatial distribution of a defined confidence interval).
...
The atmospheric correction has a range of \num{-0.04} to \SI{0.18}{mGal} at the Earth surface \parencite{Tenzer2009_AtmCorr}.
...
\label{ss:SigIs:E2014fwd_TC_g_rad}
...
Only $V_P$ and $V_S$ are provided, with a Poisson ratio ($\sigma$) fixed at~\num{0.26}.
...
Phase equilibria was modelled with the {Perple\_X} program \parencites{Connolly2005}{Connolly2009}, adopting the {hp02ver} thermodynamic data base \parencites{Powell1998_hp02}{Ghiorso2002_pMELTS}, which is a comprehensive chemical model for silicates in the lithosphere, appropriate for depths less than \SI{440}{\kilo \metre}.
...
Pressure values where computed as purely lithostatic pressure, by depth-integration of the density of the overlying layers and of the constant \SI{3300}{\kilo \gram \per \cubic \metre} lithospheric mantle density (prior to conversion).
...
\begin{figure}
    ...
    \caption[Density in the lithospheric mantle (LID) after conversion.]{
        ...
        \textbf{Bottom:}~lithospheric mantle density, obtained by velocity to density conversion.}
    \label{fig:SigIs:LID_VS-density_map}
\end{figure}

It must be noted that this conversion strategy was set up enable propagation of lithospheric mantle inhomogeneities, as sensed by a model that provide only velocities.
...
By assuming that the normal gravity arises from a `reference Earth' with ellipsoidal stratification and with no lateral inhomogeneities, we can express any density refinement to the normal gravity model as a contrast against this reference.
...
The densities of the modelled layers are thus expressed as density contrasts against a reference.

% slicing description
For these tests, the \textit{ak135} model \parencites{Kennett1995_ak135}{Montagner1996} was adopted.
The depth-density reference, in the form provided in the {NMSOP-2} manual \parencite{Bormann2002}, was tailored for these experiment by computing the average density in selected ranges of depth, as shown in Fig.~\ref{fig:SigIs:Ref_ak135}.
These ranges define a set of shells of homogeneous density.
The modelled layers were `sliced' according to these shells: in other words, an intersection was performed between the boundaries defining each layer and the reference shells boundaries (see Fig.~\ref{fig:SigIs:RefSlices}).
...
In each layer-slice the density contrast was expressed as the difference between the density prescribed by the model at those coordinates and the density of the reference shell in that depth range.
...
\begin{figure}
    ...
    \fbox{
      ...
      \begin{tabular}{rl}
        \toprule
        \multicolumn{1}{c}{\textbf{Depth}} & \multicolumn{1}{c}{\textbf{Density}} \\
        \multicolumn{1}{c}{\textbf{[\si{\kilo \metre}]}} & \multicolumn{1}{c}{\textbf{[\si{\gram \per \cubic \centi \metre}]}} \\
        \midrule
        0   & \multirow{2}{*}{2.670} \\
        10  & \multirow{2}{*}{2.920} \\
        18  & \multirow{2}{*}{3.611} \\
        ...
        60  & \multirow{2}{*}{3.523} \\
        80  & \multirow{2}{*}{3.483} \\
        100 & \multirow{2}{*}{3.446} \\
        120 & \multirow{2}{*}{3.399} \\
        165 & \multirow{2}{*}{3.348} \\
        210 & \multirow{2}{*}{3.345} \\
        260 & \multirow{2}{*}{3.389} \\
        310 \\
        \bottomrule
      \end{tabular}
    \end{tabular}

      \caption[Depths and densities from the \textit{ak135} reference model, original and discretised.]{
        \textbf{Blue}:~input depths and densities from the \textit{ak135} reference model \parencite{Kennett1995_ak135}, set at \SI{2.67}{\gram \per \cubic \centi \metre} at less than \SI{10}{\kilo \metre}.
        \textbf{Orange}:~constant density layers obtained by averaging the \textit{ak135} model in discrete depth intervals.
        These layer-discretised densities are shown in the accompanying table.}%
...
% giustifico mio modello incertezza con valori che ha ottenuto Szwillus (2019)
% altri data-aware uncertainty methods: Grad, EU, forse anche eunaseis artemieva

% considerazioni globali uncertainty, da articolo LITHO1.0
...
\begin{subappendices}
\section{Supplementary: testing the method against spatial--domain forward modelling}
...
1) the behaviour of the layer-wise modelling (section~\ref{ss:SigIs:Fwd:LayerFwd}), since it is an ``off-label'' use of the relief forward modelling method by \textcite{Wieczorek2007};
2) the difference in results with a spatial domain modelling method, given the well known issues with limited bandwidth of spectral domain methods \parencite{Hirt2014}.
...
\begin{figure} % 4 blocks, output maps
    ...
    \caption[Forward modelled output of the spectral--spatial comparison, ``four blocks test''.]{
        ...
        In the lower left map, the spherical harmonics coefficents were synthetized up to degree and order $l_{max}$ \num{360}, with an abrupt truncation at $l_{max}$.
...
\begin{figure} % 4 blocks, deg var spectra comparison: 1 relief vs layers
    ...
    \caption[``Single-relief'' against layer-wise comparison: degree variance and difference spectra.]{
        ...
        The \textbf{lower panel} show the cumulative standard deviation spectra of the differences in radial component of gravity ($g_r$), computed as layer-wise minus single-relief.}
...
\begin{figure} % 4 blocks, deg var spectra comparison: layers vs tesseroids
    ...
    \caption[Spectral domain modelling against spatial domain modelling: degree variance and difference spectra (four blocks test).]{
        ...
        Note that no difference can be graphically appreciated between the two plotted curves.
        The \textbf{lower panel} show the cumulative standard deviation spectra of the differences in radial component of gravity ($g_r$), computed as spatial-domain minus spectral-domain.
        The cumulative standard deviation of misfit at degree 360 is \SI{0.512}{mGal}.}
...
\begin{figure} % SEDS1 test, deg var spectra comparison
    ...
    \caption[Spectral domain modelling against spatial domain modelling: degree variance and difference spectra (SEDS1 test).]{
        ...
        Note that no difference can be graphically appreciated between the two plotted curves.
        The \textbf{lower panel} show the cumulative standard deviation spectra of the differences in radial component of gravity ($g_r$), computed as spatial-domain minus spectral-domain.
        The cumulative standard deviation of misfit at degree 360 is \SI{0.016}{mGal}.
    }
