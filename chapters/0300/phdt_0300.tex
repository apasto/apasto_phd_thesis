...
The local minima of the NETC in areas of high topography are over-compensated by the crustal correction, resulting in the largest values in the distribution (above about \SI[retain-explicit-plus]{+700}{mGal}).
Quality assessment of the input data is beyond the objectives of these tests.
Anyway, this over-compensation, which is a deviation from isostatic equilibrium, can be attributed to two likely causes: either the densities and/or thickness estimated in {LITHO1.0} are erroneous, or isostatic compensation is reached at depths higher than the crust-mantle boundary.
% qui utile citazione [!!!] [!!!] [!!!]
% \parencites{Martinec1994}{Bagherbandi2012}
Without excessive speculation, the actual situation can be expected to be a superposition of the two `culprits': part a ``known-mass model omission error'', part local deviations from general models.

% LID
\subsubsection{Lithospheric mantle correction}
\label{sss:SigIs:Results:Maps:LID}
Lithospheric mantle correction, \textbf{placeholder}.

% shallow and deep
\subsubsection{Shell-wise corrections}
\label{sss:SigIs:Results:Maps:Shellwise}
Shell-wise corrections, \textbf{placeholder}.

% differenza che permane: la maggiorparte (10 mGal circa) arriva da primo strato (2670 vs 2681)
% metto tabella delle medie per ogni shell, ce l'ho quasi pronta!
