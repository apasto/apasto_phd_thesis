...
Since the reference density model (Fig.~\ref{fig:SigIs:Ref_ak135}) is based on the average densities of the input model, this means that any crustal mass under that average depth results in a negative crustal correction.
Compared to that average, localised crustal thickening is an outlier in the average of the deeper reference shells (e.g. below \SI{50}{\kilo \metre}) -- therefore, relatively few nodes of lower (crustal) density, respect to the mantle, cannot shift the average density by much.
The consequence is that deep crustal keels result in large density contrast (against the reference), thus large forward modelled gravity corrections.
...
A bi-modal distribution, superposition of an ocean and a continent member, can be identified at least qualitatively.
The gravity effect ($g_z$) in all grid-nodes over continents is consistently negative.
Over oceans are still negative, at about \SI{-70}{mGal}
