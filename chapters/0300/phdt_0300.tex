...
They were applied in the gravity-constrained thermal modelling strategy described in this thesis, in chapter~\ref{c:ThermAppl}.
The forward-modelling experiments presented in this chapter include corrections for the terrain effect (solid topography, water, and ice), sediments and other intra-crustal density variations, and inhomogeneities in the lithospheric mantle.
...
Results include the probability density function of these reductions, from which the global spatial distribution of the standard deviation was computed.
...
Following the recommendations in \textcite{Hackney2003} and \textcite{Vajda2007}, the formulation hereby presented is based on gravity disturbances, to avoid the ``Secondary Indirect Topographic Effect'' that arises when the topographic correction is applied to gravity anomalies \parencite{Meurers2017chapter}.
...
There are, however, some methodological discrepancies in the various definitions \parencites(see)()[][ch.~5]{LaFehr2012}[][ch.~6.3]{Hinze2013}.
...
Signal isolation is also addressed using spectral separation, such as the source-depth theory developed by \textcite{Bowin1983}: \textcite{Li2017} applied it to the inversion of GOCE gradients, in order to separate the integrated effect of mantle, crust and sediments in the Tibetan plateau.
It was also used, similarly, by \textcite{Afonso2019} to remove the contribution of density anomalies under \SI{410}{\kilo \metre} in their \textit{LithoRef18} global density model.
However, as \textcite{Kaban2004} pointed out, considerable overlap in long wavelengths between shallow and deep sources warrants caution in doing so.
...
Mantle stripping is affected by the same issues, and no ready-made velocity conversions of seismic models are available, as observed by \textcite{Tenzer2015}, requiring again to resort to conversion schemes \parencites(e.g.)(){Sebera2018}{Liu2018}.
...
Compared with the mean Earth radius ($R = \SI{6371}{\kilo \metre}$), a \SI{40}{\kilo \metre} deep crust--mantle boundary accounts for about 6 thousandths of $R$.
...
The transform on a $n \times 2n$ \textcite{Driscoll1994} grid is exact if $f$ is band-limited to a maximum degree of $n/2 - 1$.
...
The `normal density reference' obtained in this way (average {LITHO1.0} densities and `asthenosphere fill-in') is presented in Fig.~\ref{fig:SigIs:Ref_ak135} (black line) and in Tab.~\ref{tab:Ref_ak135}.
...
\begin{figure}
    ...
    \caption[Sketch describing the `slicing' of layers according to a reference density model.]{
        Sketch describing the `slicing' of layers according to a reference density model (a concentric depth-wise succession of constant density shells).
        ...
        Where no part of the layer intersects a reference shell, i.e. where there is no mass to model, the two surfaces coincide.}
...
% For comparison, the standard deviation of Moho contrast obtained by \textcite{Sjoberg2011} were distributed between \num{26} and \SI{90}{\kilo \gram \per \cubic \metre}.
...
\subsection[Maps of the modelled corrections and reduced NETC]{Maps of the forward modelled corrections and reduced NETC}
        ...
        \fbox{
            \begin{tabular}{c}
                \begin{overpic}[width=\textwidth]{maps/sig_isol/GGMin.pdf}
                    \put (0,65) {\textsf{\textbf{GGM $\bm{\delta g}$}}}
                \end{overpic} \\
        ...
        \fbox{
            \begin{tabular}{c}
                \begin{overpic}[width=\textwidth]{maps/sig_isol/FWD_CrustT_trim.pdf}
                    \put (0,65) {\textsf{\textbf{cons. CRUST, $\bm{g_z}$}}}
                \end{overpic} \\
        ...
        \fbox{
            \begin{tabular}{c}
                ...
                \begin{overpic}[width=\textwidth]{maps/sig_isol/BG_LithoC.pdf}
                    \put (0,65) {\textsf{\textbf{NETC, LITHO corr.}}}
                \end{overpic}
            \end{tabular}
        }
    \end{adjustbox}
...
A bi-modal distribution, superposition of an ocean and a continent member, can be identified - the continent distribution showing a much wider dispersion (histogram in Fig.~\ref{fig:SigIs:Res:HistFwdCrust}).
...
Such a positive residual may arise due to two different sources: the densities and/or thickness provided in {LITHO1.0} may be erroneous, or the observed gravity signal and the estimated depth of boundaries (such as the Moho) may be influenced by non-isostatic effects \parencite[as defined by ][]{Bagherbandi2012}.
...
% LID
\subsubsection{Lithospheric mantle correction}
\label{sss:SigIs:Results:Maps:LID}
The forward-modelled correction for the lithospheric mantle (LID) is shown in Fig.~\ref{fig:SigIs:Res:LID}, top.
...
Two shell-wise correction and corrected NETC are provided: a `shallow' one down to \SI{80}{\kilo \metre}, in Fig.~\ref{fig:SigIs:Res:Shallow}, and a `deep' one down to \SI{310}{\kilo \metre}, in Fig.~\ref{fig:SigIs:Res:Deep}.
...
% The most striking differences in the corrected disturbances are observed over oceans.
% The first-\SI{80}{\kilo \metre} corrected NETC (Fig.~\ref{fig:SigIs:Res:Shallow}, bottom), compared with crust-corrected NETC (Fig.~\ref{fig:SigIs:Res:CRUST}, bottom), shows less-positive values over oceans (average here: \SI{276.4}{mGal}, crust-only average: \SI{550.5}{mGal}).
% A similar difference can be observed for the first-\SI{310}{\kilo \metre} corrected NETC, over oceans (average here: \SI{304.8}{mGal}, lithosphere-only average: \SI{205.7}{mGal}).

% risultato quindi rispiecchia maggiormente distribuzione normale del GGMin in ingresso, faccio riferimento a histogram Fig.~\ref{fig:SigIs:Res:HistGGMin}

The global averages of both the first-\SI{80}{\kilo \metre} and first-\SI{310}{\kilo \metre} corrected NETC is positive, at \num{307} and \SI{305}{mGal}, respectively.
...
\begin{figure}[hb]
    \begin{adjustbox}{center}
        \fbox{
            \begin{tabular}{c}
                \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm,
                        width=0.8\textwidth
                    ]{maps/OC_histograms/GGMin.pdf}\\
            \end{tabular}
        }
    \end{adjustbox}
        ...
        \fbox{
            \begin{tabular}{c}
                \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm,
                        width=0.8\textwidth
                    ]{maps/OC_histograms/FWD_SedsT.pdf}\\
                    \midrule
                    \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm,
                        width=0.8\textwidth
                    ]{maps/OC_histograms/FWD_CrustT.pdf}\\
                    \midrule
                    \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm,
                        width=0.8\textwidth
                    ]{maps/OC_histograms/FWD_LIDT.pdf}\\
            \end{tabular}
        }
    \end{adjustbox}
        ...
        \fbox{
            \begin{tabular}{c}
                \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm,
                        width=0.8\textwidth
                    ]{maps/OC_histograms/BG.pdf}\\
                    \midrule
                    \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm,
                        width=0.8\textwidth
                    ]{maps/OC_histograms/BG_ShallowC.pdf}\\
                    \midrule
                    \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm,
                        width=0.8\textwidth
                    ]{maps/OC_histograms/BG_DeepC.pdf}\\
            \end{tabular}
        }
    \end{adjustbox}
...
The degree variances of the shell-wise corrections are plotted in Fig.~\ref{fig:SigIs:Spec:DegVarSD}, where the spectra of the GGM and TC are kept for reference.
The degree variance of both these corrections becomes less than the one of the GGM at SH degree \num{138}, a different pattern respect to the isolated layer-wise effect.
...
% considerable overlap with mantle flow effects (citazioni)
% Pekeris 1935, Hager 1985
% citazioni lì sopra le due: opposing effects di mantle flow e surface effects
% In favour
% complete compensation of isostatic and non-isostatic effects
% low-pass filtered non-reduced gravity disturbance
% under a compensation depth
% as argued... input models... overcorrection
...
\begin{figure}
    \begin{adjustbox}{center}
        \fbox{%
        \includegraphics[
                clip, trim=0.2cm 0.35cm 0.1cm 0.1cm,
                width=1.2\textwidth
            ]{maps/spectra/DegVar_ALL.pdf}}
    \end{adjustbox}
...
\begin{figure}
    \begin{adjustbox}{center}
        ...
        \includegraphics[
                clip, trim=0.2cm 0.35cm 0.1cm 0.1cm,
                width=1.2\textwidth
            ]{maps/spectra/DegVar_ShallowDeep.pdf}}
    \end{adjustbox}
...
An almost constant CC, at \num[retain-explicit-plus]{+0.75}, is observed from SH degree 20 onwards: this reproduces a common finding in topographic evaluation \parencites{Hirt2012TopoIso}{Hirt2010} of gravity models on Earth.
...
Failing to do so would map the omitted short wavelength gravity signal to the anomalous mass distribution sought for, resulting in artefact arising from the difference in formal resolution between input model and reductions.

Loss of terrain-gravity correlation occurs also at low degrees (less than \num{10}), where they are usually attributed to deep density inhomogeneities \parencite{Kaban2004} not associated with an expression in surface topography.
        ...
        \fbox{
            \begin{tabular}{c}
                \includegraphics[
                     clip, trim=0.2cm 0.35cm 0.1cm 0.1cm,
                     width=1.2\textwidth
                 ]{maps/spectra/XCorr_sep.pdf}\\
                \includegraphics[
                    clip, trim=0.2cm 0.35cm 0.1cm 7.5cm,
                    width=1.2\textwidth
                ]{maps/spectra/XCorr_SD.pdf}\\
            \end{tabular}
        }
    \end{adjustbox}
...
A different type of degree correlation coefficients is plotted in Fig.~\ref{fig:SigIs:Res:XCorrISO}: the NETC disturbance corrected for the first \SI{80}{\kilo \metre} shell (Fig.~\ref{fig:SigIs:Res:Shallow}, bottom) and the NETC disturbance corrected for the first \SI{310}{\kilo \metre} shell (Fig.~\ref{fig:SigIs:Res:Deep}, bottom) are cross-correlated with the non-corrected NETC (Fig.~\ref{fig:SigIs:Res:GGM_BG}, bottom).
...
The results of the reduction uncertainty propagation scheme, as described in section~\ref{s:SigIs:Impl}, are presented in terms of standard deviation ($\sigma$) of the ensemble forward-modelling results for each grid node.
Three global maps of correction-standard-deviation are plotted in Fig.~\ref{fig:SigIs:Res:UncertaintyMaps}, from the top: sediments (SEDS1, SEDS2, SEDS3), consolidated crust (CRUST1, CRUST2, CRUST3), and lithospheric mantle (LID).

A direct consequence of the simple uniform uncertainty model is the direct proportionality between uncertainty and the thickness of the modelled layers.
Dependence of depth-uncertainty with the depth of layer boundaries and the \SI{100}{\kilo \gram \per \cubic \metre} standard deviation of density uncertainty, add up.
The consequence is that the deeper and thicker a layer, the larger its contribution to uncertainty in a given location.

The scatter plots of Fig.~\ref{fig:SigIs:Res:UncertaintyScatterPlots} present this proportionality pattern, which however deviates from simple linearity.
The patterns are complicated by the concurrent contributions of 
: (1)~different composing layers (in the case of sediments and crust, 3 layers each), (2)~lateral variations in density in each layer, (3)~different contrast respect to the reference density in each shell of the reference model, (4)~large footprint of deep and far-field masses, especially in areas of sharp lateral contrasts (e.g. ocean margins).
The LID layer, which is characterised localised bodies with high contrast (cratonic keels at large depths and patches of shallow dense lithospheric mantle in oceans), deviates from linear proportionality more significantly than the sediments and crust.

To avoid over-representation of nodes at high latitudes, the markers in the scatter plots of Fig.~\ref{fig:SigIs:Res:UncertaintyScatterPlots} were obtained by re-sampling the layer thickness and uncertainty maps on the regularly-spaced global grid (\SI{2}{arc-degree} spacing) by \textcite{Davies2012}.

% note per equal-area-sampling:
% centroidi di rettangoli 2x2 gradi (all'equatore), area costante = circa 50000 km^2
% da Davies2012
...
\begin{figure} % using rasterized PDF->PNG due to size constraints
    \begin{adjustbox}{center}
        \fbox{
            \begin{tabular}{c}
                \includegraphics[width=\textwidth%
                    ]{unc_thickness/UNCvsTHK_Seds.png}\\
                \midrule
                \includegraphics[width=\textwidth%
                    ]{unc_thickness/UNCvsTHK_Crust.png}\\
                \midrule
                \includegraphics[width=\textwidth%
                    ]{unc_thickness/UNCvsTHK_LID.png}\\
            \end{tabular}
        }
    \end{adjustbox}
    \caption[Layer thickness versus propagated uncertainty scatter plots.]{Layer thickness versus propagated uncertainty (SD, one standard deviation) scatter plots.}
    \label{fig:SigIs:Res:UncertaintyScatterPlots}
\end{figure}

\FloatBarrier

The spatial distribution of uncertainty values and their surface-weighted probability density function -- as plotted in the histograms beneath the maps (Fig.~\ref{fig:SigIs:Res:UncertaintyMaps}) and in Fig.~\ref{fig:SigIs:Res:HistUncertainty} -- bears a qualitative similarity with the histograms of the forward-modelled corrections, especially for the partition pattern of oceans and continents.

The average standard deviation for the sediment correction is \SI{2.5}{mGal} (oceans: \SI{2.7}{mGal}, continents: \SI{1.9}{mGal}), with tails reaching values over \SI{10}{mGal} only directly above thick sedimentary basins.
The uncertainty in crust correction is an order of magnitude larger, with a marked ocean-continent dichotomy.
Due to the thickness-uncertainty relationship, discussed before, larger values are observed over thicker crust, therefore all the higher part of the range is covered by nodes on above-sea-level landmasses (global average: \SI{18.3}{mGal}, oceans: \SI{13.0}{mGal}, continents: \SI{32.1}{mGal}).
Unsurprisingly, the crustal correction uncertainty reaches its largest values in the same regions where the crustal correction itself has its extremes: chiefly, the Himalaian range (up to \SI{72}{mGal}).
The uncertainty of the lithospheric mantle correction loses most of the ocean-continent dichotomy, since the layer itself is not strongly correlated with surface topography.
While the maximum value is less than the one of $\sigma(\textrm{CRUST})$, the average is higher (\SI{21}{mGal}) and there is less dispersion, due to the lithospheric mantle effect lacking any localised `outliers'.

% conseguenze qui
% un range +/- 2 sigma di [media continentale]
% limitato effetto per i soli continenti [cito esempio di Kaban2004]
% migliora con relazioni locali [riferimento a capitolo paper]
% interpretando la correzione crostale come modello iniziale per crosta
% esempio per raffinare modelli sismici [valuto se opportuno citare Eshagh]
% calcolo piastra di Bouguer [citazione?] equivalente a +/- 2 sigma media continentale crosta

Taking into account a $\pm 2 \sigma$ range (covering the \SI{95}{\percent} of the values, under the normally distributed error imposed here) and crustal thickness modelling as a potential application (e.g. the case involved in chapter~\ref{c:ThermAppl}), the uncertainty introduced by the sediment correction remains relatively small if compared to other contributions.
For a `rule-of-thumb' assessment, I adopt the first-order term of the attraction of a spherical Bouguer shell ($A = 4 \pi G \rho H$, \cite{Vanicek2001}) as an approximated conversion between the uncertainty range and an `equivalent Moho depth' (arranged as $\Delta H = 4 \sigma (A) / (4 \pi G \Delta \rho)$).
Using a \SI[retain-explicit-plus]{+485}{\kilo \gram \per \cubic \metre} crust-mantle contrast \parencite{Tenzer2012contrast}, a \SI{10}{mGal} $4 \sigma$ range results in a uncertainty interval of about \SI{245}{\metre}.
For comparison, \textcite{Grad2012} reports an \SI{85}{\percent} confidence interval of $\pm$\SI{2}{\kilo \metre} for a \SI{20}{\kilo \metre} thick crust, $\pm$\SI{4}{\kilo \metre} for a \SI{60}{\kilo \metre} thick crust
(for receiver functions on the well-surveyed European Plate).
Note, however, that this error estimate disregards the uncertainties introduced by the gravity inverse problem.
% media seds = 2.5, *4 = 10 mGal
% contrasto 485 kg/m3 (Tenzer2012)
% H = deltagrav / (2 Pi rho G) = about 492 m (Bouguer plate, half of shell)

The crustal and lithospheric-mantle correction, on the other hand, are a significant source of error.
Adopting the same Bouguer-shell conversion, the average $4 \sigma$ of the crustal correction over continents (\SI{128}{mGal}) results in about \SI{3150}{\metre} of equivalent-Moho uncertainty range (again, this accounts only for the contribution of reductions).
% esempio portato: media su continenti 32 mGal *4 = 128 mGal
% come sopra, ottengo 6297 m

% potrei perdermi in esempi di inversione di sfere... ma non lo farò
% faccio invece riferimento al fatto che nel capitolo termico faccio riferimento alla sensibilità all'errore sulla crosta
% e al fatto che le "cose funzionano ancora"
% inoltre, nelle conclusioni finali, viene fatto test con tutto assieme (MA NON LO ANTICIPO QUI)
        ...
        \fbox{
            \begin{tabular}{c}
                \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm
                        width=0.8\textwidth
                    ]{maps/OC_histograms/UNC_Seds.pdf}\\
                    \midrule
                    \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm
                        width=0.8\textwidth
                    ]{maps/OC_histograms/UNC_Crust.pdf}\\
                    \midrule
                    \includegraphics[
                        clip, trim=0.9cm 0.65cm 1.2cm 0.9cm
                        width=0.8\textwidth
                    ]{maps/OC_histograms/UNC_LID.pdf}\\
            \end{tabular}
        }
    \end{adjustbox}
    \caption[Histograms of the propagated uncertainties, for SEDS, CRUST, and LID.]{
        Histograms of the propagated uncertainties ($\sigma$) of the corrections: sediments ({SEDS}), consolidated crust ({CRUST}), and lithospheric mantle ({LID}).
        The respective maps are shown in Fig.~\ref{fig:SigIs:Res:UncertaintyMaps}.
        Nodes over oceans (blue) and continents (orange) are counted separately. Their bars are plotted stacked.
    }
    \label{fig:SigIs:Res:HistUncertainty}
\end{figure}
...
%                 clip, trim=0.2cm 0.35cm 0.1cm 0.1cm,
%                 width=1.2\textwidth
%             ]{maps/spectra/DegVar_UNC.pdf}}
%     \end{adjustbox}
%     \caption[Temp caption.]{Temp caption.}
%     \label{fig:SigIs:Spec:DegVarUNC}
% \end{figure}

\FloatBarrier

\section{Conclusions}
\label{s:SigIs:Conclusions}
In this chapter I have presented a method for fast forward modelling of mass distributions in the lithosphere, relying on a layer-wise modelling strategy based on \textcite{Wieczorek2007} spherical harmonics formulas.
Forward modelled gravity corrections to the gravity disturbance, aimed at stripping gravity measurements, were computed for the complete terrain effect (according to the rock, water, and ice {Earth2014} model by \cite{Hirt2015}) and for the sediments, crust and lithospheric mantle (according to the {LITHO1.0} model by \cite{Pasyanos2014}).

In order to perform velocity-to-density conversion in the lithospheric mantle (where {LITHO1.0} does not provide the spatial distribution density), thermodynamic modelling was performed with {Perple\_X} \parencite{Connolly2005} on a simplified two-member global compositional model \parencite{Griffin2009}.

Reductions were modelled as density contrasts against an ad-hoc reference density model, discretised in concentric shells respect to the {GRS80} ellipsoid.
The reference density in each shell was computed according to {LITHO1.0} densities and a reference constant-density asthenosphere fill-in.
By doing so, the biases that exist between the input density model and commonly adopted reference earth models (e.g.~{ak135}) were not propagated to the forward modelled corrections.
This involved partitioning each layer according to the `reference-slices' and modelling the `sliced' gravity effect separately.
This procedure also provided novel grouped statistics on the average depth and densities of {LITHO1.0} layers (Tab.~\ref{tab:LithoLayersGroups}).

The forward-modelled corrections were applied to the combined satellite-only global gravity model GOCO06s \parencites{Kvas_GOCO06s_dataset}{Kvas2019GOCO06s_EGU}.
They are presented as global maps of gravity effect of the modelled layers (i.e. attraction of the modelled masses) and of the incrementally corrected gravity disturbance of the global gravity model.
In addition to layer-wise corrections (grouped in sediments, consolidated crust, and lithospheric mantle), two selected `shell-wise' corrections are included.
The `shell-wise' corrections, instead of modelling masses up to the bottom boundary of the bottom-most layer, include all the modelled layers plus the fill-in asthenosphere up to a constant ellipsoidal depth.
Biases due to the non-modelled volumes are therefore avoided.
The residual gravity disturbance obtained from such corrections could be employed in inverse modelling of lithospheric and sub-lithospheric `anomalous masses' respect to the model used for correction and the fill-in reference asthenosphere.

Maps are accompanied by histograms with constant-area weighted counts.
Global averages have also been provided, separated for nodes over oceans and continents, enabling a quantitative separation of the already apparent ocean-continent dichotomy in the modelled layers.
The corrections obtained highlight some of the issues regarding the `known-masses' model {LITHO1.0}, particularly regarding isostatic compensation of topographic masses.
Two aspects, over-correction in areas of very thick crust and non-modelled compositional effects of cratonic keels, could be identified and are briefly discussed.

A spectral-domain assessment of the modelled quantities was also performed.
Degree cross-correlation coefficients between the input gravity model and the obtained corrections were computed.
Leaving isostatic analysis in the spectral domain aside -- it would call for a more rigorous analysis \parencites[e.g.][]{McKenzie1997}{Kaban2004} -- the assessment performed here provided an useful estimate on the effective bandwidth of the {LITHO1.0} model, which decayed to near-zero correlation coefficients at spherical harmonics degrees less than what its \SI{1}{arc-degree} nominal resolution could provide.
This finding suggests that localised assessment of degree cross-correlations (e.g. with multitaper spectral estimation, such as the method by \cite{Wieczorek2007multitaper}) could provide interesting metrics on the actual resolution of global crust/lithosphere models with undisclosed data sources.

Cross-correlation of the terrain correction with the input global gravity, an assessment that has been already performed for other GGMs and terrain models \parencite[e.g.][]{Hirt2012TopoIso}, reminded of the loss of spectral power that satellite-only GGMs have at higher degrees -- this calls for adequate attenuation of the forward-modelled solid-Earth reductions.

Propagation of errors in the depth of layer boundaries and layer densities was carried out with a Monte Carlo technique \parencite{Aster2018}, a parallel computation scheme and a memory-lean partition strategy.
It has proven successful in providing a propagated uncertainty estimate, quantified with the standard deviation of the correction values in each node of the synthesized global gravity maps.
The input error model that has been adopted is based on a simple depth-dependent error in depth estimation and in a fixed standard deviation value for the normally-distributed error in density.
Anyway, the method that I have devised may be easily scaled to more complex models of the spatial distribution of errors, based on quality and availability of data \parencite[such as in the regional model by][]{Molinari2011} and/or a geostatistics-based estimation of variance \parencite[e.g.][]{Szwillus2019}.
Error covariance between nearby nodes is also an aspect that warrants further investigation.

% accenno a prossimo capitolo?

This chapter is accompanied with a supplementary section (\ref{s:SigIs:Test}), in which the adopted spectral-domain forward modelling method is compared with spatial-domain forward modelling, in a set of synthetic test cases.
...
% LISTA punti 1-5 in Kaban 2004 : isostatic anomalies sources
% 1. Density heterogeneities in the upper crust (internal load), which are unknown and not considered in the isostatic model, e.g. sedimentary layers or deviations of the upper crust density from the nominal value.
% 2. Density heterogeneities in the lower crust/upper mantle that differ from the adopted compensation scheme (e.g. Airy).
% 3. Deep density heterogeneities located below the level of the isostatic compensation.
% 4. disturbances of the isostatic equilibrium which can also be attributed to: (a) local disturbances due to the rigidity of the lithosphere (regional compensation of local load); (b) deep dynamic effects (dynamic sup- port of topography by mantle convection); and (c) visco-elastic response to time-varying loads (e.g. postglacial rebound).
...
% "formal resolution"
% conseguenza: correzioni devono tener conto di questo
% sebbene potenza a tali gradi sia limitata
% quando confrontata con modelling a scala globale (questi esperimenti)
% non-necessaria per applicazioni geofisiche a questa scala
...
Set on a sphere with radius equal to WGS84 semi-major axis (\SI{6378.137}{\kilo \metre}), two blocks rise above it, the remaining two below it, arranged in a checker board fashion.
...
The resulting relief map is plotted in Fig.~\ref{fig:SpatSpecComp:4B_DepthMap}, zoomed in on the area covered by the blocks (centred on \SI{47}{\degree N}, \SI{47}{\degree E}).
...
A section along the \SI{48}{\degree N} parallel is plotted in Fig.~\ref{fig:SpatSpecComp:4B_sections}, which shows the three ways in which the mass distributions have been expressed:
\begin{enumerate}
...
\item[c)] spatial-domain \textbf{tesseroids} discretisation: the relief was turned into discrete spherical tesseroids, sized with the same grid spacing as the input grid and centred on the grid nodes (\textit{grid registration}). Forward modelling was carried out with the Tesseroids code \parencites{Uieda2016}{UiedaTesseroids}.
\end{enumerate}
...
Applying a linearly decreasing weight (from 1 to 0) to all coefficients from degree 200 to 360 results in the differences plotted in the lower right panel.
...
The results, expressed in terms of power spectra (degree variances of the potential) and spectrum of $g_z$ differences (cumulative degree standard deviation) are shown in Fig.~\ref{fig:SpatSpecComp:4B_dv_spec_relief}.
...
These three modifications simplified the generation of the corresponding tesseroids-discretised model and computation points grid, thus avoiding a potential source of discrepancy in the direct comparison.
...
Misfit levels, both in a synthetic 2$\times$2 checker board test and with a realistic global layer, are small enough to exceed the application requirements: they lie below the uncertainties involved in the input mass distribution data.
