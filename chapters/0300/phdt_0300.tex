...
\begin{figure}
    ...
    \end{tabular}
      ...
      \caption[Depths and densities from the \textit{ak135} reference model, original and discretised.]{
        \textbf{Blue}:~input depths and densities from the \textit{ak135} reference model \parencite{Kennett1995_ak135}, set at \SI{2.67}{\kilo \gram \per \cubic \metre} at less than \SI{10}{\kilo \metre}.
...
\begin{figure}
    ...
    \end{adjustbox}
    \caption[Sections trough the synthetic mass distribution in the three tested three tested modelling methods]{
        Sections trough the synthetic mass distribution in the three tested three tested modelling methods:
        \textbf{a)}~one relief against a reference sphere of radius $D$: input for the unmodified spectral-domain method of \textcite{Wieczorek2007};
        ...
        \textbf{c)}~tesseroids discretisation (\SI{0.0625}{\degree}$\times$\SI{0.0625}{\degree}), input for the spatial-domain method.
        All heights are expressed in respect to a reference sphere with radius equal to \SI{6378.137}{\kilo \metre}.
        The red-filled areas denote positive input densities and the blue-filled ones denote negative densities.}
    \label{fig:SpatSpecComp_sections}
\end{figure}
