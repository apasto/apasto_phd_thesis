...
The scatter plots of Fig.~\ref{fig:SigIs:Res:UncertaintyScatterPlots} present this proportionality pattern, which however deviates from simple linarity.
The patterns are complicated by the concurrent contributions of 
: (1)~different composing layers (in the case of sediments and crust, 3 layers each), (2)~lateral variations in density in each layer, (3)~different contrast respect to the reference density in each shell of the refence model, (4)~large footprint of deep and far-field masses, especially in areas of sharp lateral contrasts (e.g. ocean margins).
...
The uncertainty in crust correction is an order of magnitude larger, with a marked ocean-continent dicothomy.
...
The uncertainty of the lithospheric mantle correction loses most of the ocean-continent dicothomy, since the layer itself is not strongly correlated with surface topography.
...
For comparision, \textcite{Grad2012} reports an \SI{85}{\percent} confidence interval of $\pm$\SI{2}{\kilo \metre} for a \SI{20}{\kilo \metre} thick crust, $\pm$\SI{4}{\kilo \metre} for a \SI{60}{\kilo \metre} thick crust
(for receiver functions on the well-surveryed European Plate).
...
This involved parititioning each layer according to the `reference-slices' and modelling the `sliced' gravity effect separately.
...
The residual gravity disturbance obtained from such corrections could be empoyed in inverse modelling of lithospheric and sub-lithospheric `anomalous masses' respect to the model used for correction and the fill-in reference asthenosphere.

Maps are accompanied by histograms with constant-area weighted counts.
Global averages have also been provided, separated for nodes over oceans and continents, enabling a quantitative separation of the already apparent ocean-continent dicothomy in the modelled layers.
