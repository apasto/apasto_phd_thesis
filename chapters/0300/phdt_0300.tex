...
\begin{figure}
    ...
    \caption[Terrain correction: sketch of the solid topography and ocean bathymetry stripping.]{
        ...
        The hatched area 
            \includegraphics[width=0.75\baselineskip]{symbol_hatched_out.pdf}
        ...
        The teal-coloured area 
            \includegraphics[width=0.75\baselineskip]{symbol_water_out.pdf}
...
\subsection{Sediments and crust}
...
Thickness and properties of the crust and lithospheric mantle in {LITHO1.0} are the result of targeted--grid--search based inversion of surface wave dispersion maps.
...
% Harmonising the data available at a global scale, even at a target resolution of \SI{1}{\degree}, is still

% In addition, in order to use such a model for gravity data reduction,

% either by using a purely non-gravity dataset
% or by integrating the
% multi--observable strategy

% The recent work by \textcite{Szwillus2019}
% provides a

There are two features that are particularly relevant for applications such as the one hereby described:
...
(2) the estimation of uncertainty, which could then be employed in a realistic uncertainty propagation to the forward modelled gravity field.

%Competing candidates for an input model
% elenco, es modelli
% e cito lavori recentissimi
%with the issue of global harmonisation

\begin{center}
    \textbf{[Da inserire: ragioni per scelta LITHO1.0, modelli simili esistenti, differente approccio di \textcite{Szwillus2019} e \textcite{Afonso2019}]}
\end{center}

% ragioni per preferire litho
% sensibilità bulk properties delle surface wave
% integrata con crust
% tectonic regularisation in crust
% integrazione con sediment model
% different assumptions (oceans, descrivo)
% con quella stima di incertezza e differenza -> giustifico mio modello incertezza

% data-aware uncertainty methods: Grad, EU
% forse anche eunaseis artemieva

% take-home message: il metodo è adattabile a modelli complessi, questa è proof of concept

\begin{center}
    \textbf{[Da inserire: discorso su proprietà derivate, non indipendenti]}
\end{center}
% discorsi su proprietà derivate, fissate, rapporti
%The three properties provided in the model ($V_P$, $V_S$, and $\rho$) were not independently estimated.
...
Density in the lithospheric mantle is provided as a constant value (\SI{3300}{\kilo \gram \per \cubic \metre}).
The extraction of a velocity-converted density is the subject of the following section (\ref{ss:SigIs:InModels:SCLM}).
...
\subsection{Lithospheric mantle}
\label{ss:SigIs:InModels:SCLM}
% \SI{3300}{\kilo \gram \per \cubic \metre}
% sigma (poisson ratio) in LID è fissato a 0.26
% Vs/Vp = sqrt( (0.5 - sigma) / (1 - sigma) )
% da Sheriff2002
% quindi Vs/Vp in LID = 0.5695
