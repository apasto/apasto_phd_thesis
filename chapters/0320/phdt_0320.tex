...
To assure convergence of the SH series, modelling is typically performed outside the \textit{Brillouin sphere} \parencite{Moritz1980}, i.e. a sphere encompassing all the field-generating mass, concentric with the spherical coordinate system \parencites{Hu2015}{Hirt2017}.
...
It is described in detail in the following section, \ref{sss:SigIs:Fwd:SHfwd}.
...
Defining a \textit{layer} as a volume of radially constant density $\rho(\lambda, \phi)$, bound by two surfaces expressed by geocentric radii $t(\lambda, \phi)$ and $b(\lambda, \phi)$, respectively top and bottom, its resulting potential is equal $W_{b}^{t}$ to the following difference:
\begin{equation}
...
Therefore, the potential of a layer with two arbitrary bounding surfaces can be forward modelled by computing the difference between two relief referred to the same sphere, $W_{b}^{D}$ and $W_{top}^{D}$.
A graphical representation of this difference is shown in Fig.~0. % mettere figura
...
% QUI passo a mio metodo, sovrapposizione
% dove metto benchmark? accenno?
% inoltre: effetto D troppo distante (nei test, spettralmente... magari altra sezione?)
% devo decidere dove mettere power, cross power, deg variances
% ha senso appesantire i results?
% meglio forse definirli in una subsection qui
