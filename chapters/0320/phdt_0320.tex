...
These experiments rely on the {LITHO1.0} model by \textcite{Pasyanos2014}.
They provide a global model of the lithosphere, from the topography to the base of lithospheric mantle (lithosphere--asthenosphere boundary, LAB), parametrized vertically in a series of `geophysical layers'.
Horizontally (i.e. referring to the Earth surface) data is discretised on a \SI{1}{\degree} icosahedron--based tessellation.
...
% brief description of the model motivation - stemming from Crust1.0
% \parencite{Laske2012Crust10}
However, care must be taken

as the authors stated, it is by no means a
derived properties
imposed constrains

a satisfying degree of independence from gravity data
apart from Antarctica
where they used a gravity-based crustal thickness

no uncertainty information is provided
own simplified error model
described in section~\ref{ss:SigIs:Impl}

% fine descrizione, parte mia
% circa
% "air, water, ice" discarded [...]
...
As mentioned for the terrain correction, this data also needs to be gridded to in a format compatible with the chosen gravity forward modelling: an equally spaced $n \times 2n$ grid, consistent with \textcite{Driscoll1994} sampling theorem.
Triangulation from the tessellated data to cartesian coordinates was performed using the {Stripy} package \parencites{Moresi2019}{Moresi2019_zenodo} and {litho1pt0} \parencite{Moresi2019_litho1pt0}, a Python interface to {LITHO1.0}.
