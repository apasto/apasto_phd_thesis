...
\subsubsection{Forward modelling method}
\label{sss:SigIs:Defs:Fwd}

Forward modelling the gravity field resulting from a mass distribution involves integrating its gravitational effects, thus evaluating the so called ``Newton integral'' \parencite{HofmannWellenhof2006}.
Methods to do so are either \textit{space--domain} or \textit{spectral--domain} forward modelling methods \parencite{Kuhn2005}.

In the first case, the direct integration of masses is computed: volumes are decomposed (\textit{discretised}) in elementary bodies for which a solution to the Newton integral exists.
...
To assure convergence of the SH series, modelling must be performed outside the \textit{Brillouin sphere} \parencite{Moritz1980}, i.e. a sphere encompassing all the field-generating mass, concentric with the spherical coordinate system \parencites{Hu2015}{Hirt2017}.
Spectral--domain methods are commonly adopted for global computations, where they are considered less time consuming than space--domain methods, at least for degrees less than about 1000 \parencite{Kuhn2005}.
Computing higher maximum degrees of the SH series expansion, points inside the Brillouin sphere, and rugged topography are three factors that influence issues of numerical accuracy and divergence of spectral forward modelling methods \parencite{Hirt2017}.
...
Expressing the potential $W$ at a reference radius $R_0$ as a sum of spherical harmonic functions:
\begin{equation}
    \label{eq:SHfwd:pot}
    W(h, \lambda, \phi) =
    \frac{GM}{r}
    ...
    C_{lm} Y_{lm} (h, \lambda, \phi)
\end{equation}
where $G$ is the gravitational constant, $M$ is the total mass, 

The coefficients $C_{lm}$ are obtained from the following:
\begin{equation}
...
% qui: faccio riferimento a power, cross power, in Wieczorek2007 (treat geoph)
