...
We extracted them from the global {LITHO1.0} model \parencite{Pasyanos2014}, adopting the average Moho depth in the study area (43.9~\si{\kilo \metre}) as $D$ and a 4-degree low-pass filtered Moho contrast, as computed in section \ref{ss:GravTopoIso}.
...
When tested in our forward modelling setup, the difference in terms of Moho heat flow of a thermal-LAB temperature of 1300~\textdegree C versus 1200~\textdegree C is 1.44~\si{\milli \watt \per \square \metre}, for the same 150~\si{\kilo \metre} deep LAB ($Q_M$ of 17.05 and 15.61~\si{\milli \watt \per \square \metre}, respectively).
...
The Moho estimate that we obtained from inversion exhibits a thickening under the East Carpathians up to 51.7~\si{\kilo \metre} and up to 46.4~\si{\kilo \metre} under their north-west portion.
The two sectors are interrupted by a relatively shallow saddle (44.2~\si{\kilo \metre}) at 49\textdegree~N 22\textdegree~E.
...
Crustal thinning  in the Caledonides Foredeep (CDF) and the Baltic Basin (BB), in the north-west sector of the area, reaches 38.0~\si{\kilo \metre}.
...
Relatively thin crust (not exceeding 41.1~\si{\kilo \metre}) persists eastwards of this local minimum, following the basin shape.
It is interrupted westwards, instead, by a 48~\si{\kilo \metre} thick lineament orthogonal to the basin longitudinal axis.

We observe a thin crust under the Pannonian Basin (PB, south-west corner of the study area), never exceeding 41.8~\si{\kilo \metre} and reaching a local minimum of 38.9~\si{\kilo \metre} at 48\textdegree~N 22.75\textdegree~E.
...
The latter alternates between 40 and 48~\si{\kilo \metre} of thickness, on 120~by~120~\si{\kilo \metre} tiles.
We plotted the difference between fitted RHP, basal heat flow, and temperature at 30~\si{\kilo \metre}.
...
As a reference for the alternating pattern, the south-west corner is a thin tile (40~\si{\kilo \metre} thick crust).

\begin{figure}
	\begin{adjustbox}{center}
	\fbox{\includegraphics[width=1.35\textwidth]{./0400/CheckMoho_arrays.pdf}}
	\end{adjustbox}
	\caption{Checkerboard Moho test. Differences in bulk crustal RHP ($\Delta A$), basal heat flow ($\Delta Q_M$), and temperature at 30~\si{\kilo \metre} depth ($\Delta T(30~\mathrm{km})$) are expressed as flat Moho (44~\si{\kilo \metre}) minus checkerboard Moho (40 to 48~\si{\kilo \metre}, 120~\si{\kilo \metre} square tiles).}
...
The latter is defined by two segments: first from the surface to a mid-crustal boundary (22~\si{\kilo \metre}) set at 550~\textdegree C, then from there to the 1200~\textdegree C LAB.
